...
const MovieDetails = () => {
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 xl:px-34 pt-30 md:pt-50'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
         <img src={show.movie.poster_path} alt='' className='max-md:mx-auto 
           ...
           <div className='flex flex-col gap-3'>
            ...
            <h1 className='text-4 xl text-balance max-w-96 font-semibold'>{show.movie.title}</h1>
            <div className='flex items-center gap-2 text-gray-300'>
              <StarIcon className='w-5 h-5 fill-primary text-primary'/>
                 {show.movie.vote_average.toFixed(1)} User Rating
            </div>
            <p className='text-gray-400 mt-2 text-sm leading-tight max-w-xl'>{show.movie.overview}</p>
            <p>{timeFormat(show.movie.runtime)} • {show.movie.genres
              .map(genre => genre.name).join(', ')} • {show.movie.release_date.split('-')[0]}</p>
              ...
              <div className='flex items-center flex-wrap gap-4 mt-4'>
                ...
                <button className='bg-gray-700 rounded-full transition active:scale-95 cursor-pointer p-2.5'>
          ...
          {show.movie.casts.slice(0,12).map((cast, index) => (
            <div className="flex flex-col items-center text-center" key={index}>
              <img src={cast.profile_path} alt='' 
                className='md:h-20 aspect-square h-20 object-cover rounded-full' />
