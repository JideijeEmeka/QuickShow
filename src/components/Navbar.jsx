  ...
  return (
    <div className='fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
        ...
        <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg 
            ...
            <XIcon className='md:hidden absolute cursor-pointer w-6 h-6 top-6 right-6' onClick={() => setIsOpen(!isOpen)}/>
        ...
        <div className='flex items-center gap-8'>
            ...
            {
                !user ? (
                    <butto  onClick={() => openSignIn()} className='px-4 py-1 sm:px-7 sm:py-2 bg-primary 
                     hover:bg-primary-dull transition rounded-full font-medium cursor-pointer'>Login</butto>
                ) : (
                    <UserButton>
        ...
        <MenuIcon className='max-md:ml-4 cursor-pointer md:hidden w-8 h-8' onClick={() => setIsOpen(!isOpen)}/>
    </div>
  )
}
