    ...
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl 
       hover:-translate-y-1 transition duration-300 w-66'>
        ...
        <img onClick={() => {navigate(`/movie/${movie._id}`); scrollTo(0,0)}} 
          src={movie.backdrop_path} alt='' className='w-full h-52 object-cover 
          rounded-lg object-bottom-right cursor-pointer'/>
            ...
            {new Date(movie.release_date).getFullYear()} • {movie.genres.slice(0,2)
             ...
             <div className='flex items-center justify-between mt-4 pb-3'>
                <button onClick={() => {navigate(`/movie/${movie._id}`); scrollTo(0,0)}} 
                 className='bg-primary hover:bg-primary-dull transition 
                 rounded-full font-medium cursor-pointer px-4 py-2 text-xs'>
                    Buy Tickets
                </button>
